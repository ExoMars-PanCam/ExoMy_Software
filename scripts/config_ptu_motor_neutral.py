...
def update_config_file(ptu_pwm_neutral_dict):
    output = ''
    with open(config_filename, 'rt') as file:
        for line in file:
            for key, value in ptu_pwm_neutral_dict.items():
                if(key in line):
...
╚══════╝╚═╝  ╚═╝ ╚═════╝ ╚═╝     ╚═╝   ╚═╝       ╚══════╝╚═╝ ╚═════╝ 
    ...
    print(
        '''
This script helps you to set the neutral pwm values for the ptu motors.
You will iterate over all ptu motors and set them to a neutral position.
The neutral position is defined as the ptu looking directly forwards of the Rover.
...
------------------------------------------------------------------------------
    ...
    for pin_name, pin_value in ptu_motor_pins.items():
        ...
        print('Set ' + get_position_name(pin_name) + ' ptu motor: \n')
        while(1):
            # Set motor
